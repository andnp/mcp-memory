...
async def test_search_can_wait_for_queue_backed_embedding_repairs(db_manager) -> None:
    ...
    assert work_items.list_items(family_key=WORK_FAMILY_MEMORY_EMBEDDING_REPAIR, status="running") == []
...
def test_search_without_worker_keeps_best_effort_results_and_queues_repairs(db_manager) -> None:
    ...
    assert results
    assert results[0].memory_id == record.id
    queued_items = work_items.list_items(family_key=WORK_FAMILY_MEMORY_EMBEDDING_REPAIR, limit=10)
    health = service.get_health()
    assert queued_items
