...
def test_management_service_force_enqueues_open_cleanup_task(db_manager) -> None:
    repository = RelationalMemoryRepository(db_manager)
    task_queue = SQLiteTaskQueue(db_manager)
    service = _build_management_service(
        db_manager,
    ...
    assert len(task_queue.list_open_tasks_any_workspace("memory-curator")) == 2


def test_management_service_force_enqueues_cleanup_alias(db_manager) -> None:
    repository = RelationalMemoryRepository(db_manager)
    task_queue = SQLiteTaskQueue(db_manager)
    service = _build_management_service(
        db_manager,
    ...
    first = service.enqueue_background_task("graph-linker", force=False)
    second = service.enqueue_background_task("memory-curator", force=False)
    forced = service.enqueue_background_task("graph-linker", force=True)
    ...
    assert forced["task"]["id"] != first["task"]["id"]
    assert first["redirected_from_task_name"] == "graph-linker"
    assert forced["redirected_from_task_name"] == "graph-linker"
    assert len(task_queue.list_open_tasks_any_workspace("memory-curator")) == 2
