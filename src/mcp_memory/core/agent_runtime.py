...
def build_default_task_handlers(
    ...
    *,
    ...
    return {
        ...
        CONFLICT_DETECTOR_TASK_NAME: lambda ctx, task: handle_conflict_detector_task(ctx, task, None),
        DEFRAGMENTER_TASK_NAME: lambda ctx, task: handle_defragmenter_task(ctx, task, scoped(ctx, DEFRAGMENTER_TASK_NAME, task)),
        DEDUPLICATOR_TASK_NAME: lambda ctx, task: handle_deduplicator_task(ctx, task, None),
