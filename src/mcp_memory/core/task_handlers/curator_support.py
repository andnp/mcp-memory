...
def build_json_tool_loop_prompt(
    ...
    *,
    ...
    return (
        ...
        "4. Build a shortlist of concrete possible mutations and score each one roughly for confidence and impact. Execute every safe candidate that is high-confidence and at least medium-impact instead of merely reporting it.\n"
        "5. A local read of the current batch alone is not enough when suspicious records exist; either mutate the nearby cluster or make an explicit no-op decision after adjacency review.\n"
        ...
        "Treat the seed memories as a starting frontier, not a hard boundary; widen only for nearby duplicates, contradictions, or oversized clusters.\n"
        ...
        "Before stopping, check once more for any adjacent worthwhile maintenance action. No-op is fine only when no concrete safe cleanup remains above the high-confidence/medium-impact bar, and your final summary must make clear whether adjacency review was actually performed.\n"
